...
var ajax=function(url,callback,err)
{
	...
	httpRequest.onerror=function(e){if (err) err(e);}
...
(function(){
	...
	*/
	...
	Game.registerMod=function(id,mod)
	{
		...
		{
			if (!App && Game.Win && mod.name != 'TWCNClickerCN') Game.Win('Third-party');
			mod.init();
	...
	Game.launchMods=function()
	{
		...
		var mods = Game.sortedMods.length;
		if (Game.mods.TWCNClickerCN) mods--;
		if (!App && mods > 0) Game.Win('Third-party');
	}
...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			if (!App) Game.GrabDataResponse('{"herald": 100}');
...
window.onload=function()
{
	if (!Game.ready)
	{
		var loadLangAndLaunch=function(lang)
		{
			...
			LoadLang('loc/EN.js',function(lang){return function(){
				...
				LoadLang('loc/'+lang+'.js',function(){
					var launch=function(){
						Game.LoadMod("CookieClickerCNMod/lang.js?v="+Game.CNMODInfo?.ModVersion, () => {
							Game.LoadMod("CookieClickerCNMod/main.js?v="+Game.CNMODInfo?.ModVersion, () => {
								Game.Launch();
		...
		ajax("CookieClickerCNMod/info.txt", function (res) {
			try {
				data = JSON.parse(res);
				Game.CNMODInfo = data;
				loadLangAndLaunch('ZH-CN');
			} catch {
				loadLangAndLaunch('ZH-CN');
			}
		}, function () {
